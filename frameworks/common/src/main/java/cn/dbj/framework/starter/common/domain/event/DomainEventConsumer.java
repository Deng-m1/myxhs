...
import static cn.dbj.framework.starter.common.toolkit.MyTaskRunner.newTaskRunner;
...
public class DomainEventConsumer {
    ...
    public void consume(DomainEvent domainEvent) {
        ...
        MyTaskRunner taskRunner = newTaskRunner();

        for (DomainEventHandler handler : handlers) {
            try {
                if (handler.canHandle(domainEvent)) {
                    boolean consume=domainEventDao.updateConsumed(domainEvent);
                    if (consume)handler.handle(domainEvent, taskRunner);
                }
            } catch (Throwable t) {
                hasError = true;
